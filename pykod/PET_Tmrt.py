#Adjusted code by Noah Tingbratt, orginally from Fredrik Lindberg
...
def Solweig_2015a_metdata_noload(year,doy,hour,minu, location, UTC=0):
    ...
    while sunmaximum <= 90. - sunmax['zenith']:
                ...
                YMDHM = YMD+datetime.timedelta(days=(60*10-location["longitude"]*4)/1440.0 + fifteen)
    ...
    return None, altitude, azimuth, zen, jday, None, _, altmax
...
def Solweig1D_2020a_calc(Fside, Fup, Fcyl,
    ...
    #view factor where vegetation is in view before buildings.
    ...
    _, altitude, azimuth, zen, jday, _, _, altmax = Solweig_2015a_metdata_noload(year,doy,hour,minu, location, 0)
    ...
    if altitude > 0: # # # # # # DAYTIME # # # # # #
        ...
        Tg = 0.37 * altmax * np.sin((((hour/24+minu/24/60+location["longitude"]/360+1)//1 - SNUP / 24) / (15 / 24 - SNUP / 24)) * np.pi / 2) -3.41 # 2015 a, based on max sun altitude
        ...
        Kup = 0.15 * (radI * np.sin(altitude * (np.pi / 180.))) + dRad #underlag med albedo 0.15
